...
class TinyTracker(BaseTracker):
    ...
    def __init__(self, argv):
        super(TinyTracker, self).__init__(argv)
            ...
            for i, (frame_paths, frame_bboxs, frame_dim) in enumerate(zip(frame_paths_dirs, frame_bboxs_dirs, frame_dim_dirs)):
                ...
                for j in range(sequence_size - self.SEQUENCE_LENGTH):
                    for k in range(self.SEQUENCE_LENGTH):
                        ...
                        x_bbox[idx, k, :] = np.array([det_x_in, det_y_in, det_w_in, det_h_in], dtype='float32')
                        y_bbox[idx, k, :] = np.array([det_x, det_y, det_w, det_h], dtype='float32')
