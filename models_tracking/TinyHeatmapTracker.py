...
class TinyHeatmapTracker:
    ...
    def load_detection_model(self):
        if self.detection_model=='YOLO':
            self.model_detector = YOLO([self.cpu_mode, self.dgpu_id]) #load detection nework on gpu_id=2 while tracker on 1
            ...
            self.model_detector = FasterRCNN([self.cpu_mode, self.dgpu_id]) #load detection nework on gpu_id=2 while tracker on 1
            self.INPUT_FEAT = 2048
            self.LSTM_UNITS = 1024
            self.SEQUENCE_LENGTH = 6
    ...
    def load_tracker_model(self):
        ...
        det_input = Input(shape=(self.SEQUENCE_LENGTH, self.HEATMAP_SIZE), dtype='float32', name='detection_bbox_input')
    ...
    def get_batch(self, argv):
        ...
        x_bbox = np.zeros((self.batch_size, self.SEQUENCE_LENGTH, self.HEATMAP_SIZE), dtype='float32')
        y_bbox = np.zeros((self.batch_size, self.SEQUENCE_LENGTH, self.HEATMAP_SIZE), dtype='float32')
            ...
            for i, (frame_paths, frame_bboxs, frame_dim) in enumerate(zip(frame_paths_dirs, frame_bboxs_dirs, frame_dim_dirs)):
                ...
                for j in range(sequence_size - self.SEQUENCE_LENGTH):
                    ...
                    for k in range(self.SEQUENCE_LENGTH):
                        ...
                        y_dets[idx, k, :] = generate_heatmap_feat(det_x - det_w/2.0, det_y - det_h/2.0, det_w, det_h)
    ...
    def train(self, train_data_dirs, val_data_dirs):
        ...
        model_checkpointer = ModelCheckpoint(filepath='weights/MODEL_TinyHeatmapTracker_' + self.detection_model + '.h5',
                                        monitor='val_loss',
                                        mode='min',
        ...
        weights_checkpointer = ModelCheckpoint(filepath='weights/WEIGHTS_TinyHeatmapTracker_' + self.detection_model + '.h5',
                                        monitor='val_loss',
                                        mode='min',
        ...
        csv_logger = CSVLogger('logs/TinyHeatmapTracker_' + self.detection_model + '.log', append=False)
